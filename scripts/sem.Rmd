...
```{r}
ext_ont_tot_effects <- tot.eff(modsum, response = "extreme")
...
theme_bw()+
  ...
  labs(title = "Median Burn Severity - All Fires") + 
...
theme_bw()+
  theme(axis.title = element_text(size = 14, family = "Helvetica"), 
    ...
    panel.background = element_blank(),
        panel.border = element_rect(colour = "black", fill=NA, linewidth=1)) + 
  theme(legend.position ="top") + xlab("Standardized Effects")+
  scale_colour_viridis_d("Effect Type") +
  labs(title = "Burn Severity Extremes - All Fires") #scale_y_discrete(position = "right") #y to the right to make proper pannel of plots
...
library(gridExtra)
library(patchwork)
ext.p.all + med.p.all + guide_area() + 
