...
source("/Users/jgoldman/Work/PhD/forest-snow-fire-interactions/scripts/sem-table-function.R")
...
**What we found**: We found that the relationship between forest structure and burn severity was
...
interaction between these two factors on wildfire burn severity remain unclear. The lack of studies
concerning the relationship between snow cover, forest structure and burn severity may be attributed
to the lack of available data. However, remotely sensed burn severity, forest structure and snow
cover data products now allow us to examine these questions at broad spatial scales. Although
remotely sensed data may sacrifice accuracy at finer spatial scales, they are useful when the goal
is to identify potential driver-response relationships at coarser spatial and temporal scales.
Previous studies have used remotely sensed snow cover to look at the effects of forest structure on
snow cover [@gleason.etal2019] and remotely sensed burn severity products to examine the effects of
forest structure on burn severity [@san-miguel.etal2020], yet to our knowledge no study has examined
these data together.

Using remotely sensed data products, the primary goal of this study is to illustrate how forest
structure and snow cover interact to influence wildfire burn severity in the boreal forest of
Ontario. We use structural equation modelling to assess the strengths of the direct and indirect
relationships between forest structure and snow cover dynamics on wildfire burn severity. We fit
models at different spatial scales to compare how the strengths of these relationships differ across
gradients in climate and forest structure. Specifically, we ask (1) how does stand structure and
snow cover interact to influence wildfire burn severity in the boreal shield? (2) do the strength of
the direct and indirect relationships differ based on ecoregion and fire seasonality? (3) do snow
cover dynamics influence within fire variability in burn severity?

As climate warming increases across the globe, and especially at northern latitudes, it is
imperative to understand how future changes in snow cover interact with lanscape-scale forest
structure to influence wildfire burn severity. Such information can greatly improve our knowledge as
to the combined impacts of global change and forest structure, and how they may contribute to
wildfire severity in the future. Additionally, this information can inform forest management
strategies for reducing the impacts of snow cover loss through structural management regimes and
helping understand wildfire hazard.

**last sentence unclear** - *reducing the impacts of snow cover loss to mitigate the potential
negative effects of snow cover on wildfire, and to further our understanding of wildfire hazard.*

# Methods

*Study Area*

Our study was focused on fires that burned between 2002-2019 in the boreal shield ecozone of
Ontario. Between 2008 and 2018, over a third of the wildfires that burned in the boreal forest of
Canada occurred in the Ontario's boreal shield ecoregion [@coops.etal2018]. There is a climate
gradient from west to east which separates the ecozone into two ecoregions, boreal shield west (BSW)
and boreal shield east (BSE). The BSW is drier compared to the more humid BSE, and is a more open
jack pine (*picea banksiana*) dominated area compared to the dense black spruce (*picea mariana*)
found in the east. Typically, studies that look at wildfires in the boreal shield ecozone compare
the BSW and BSE directly, or sites within these regions, because the differences in climate and
forest structure are thought to influence wildfire dynamics [@beverly.martell2005 @james.etal2017].
...
of fires that burned in the boreal shield west ecozone (n = 284). To compare differences in
...
free date (Table \@ref(tab:s1) @ref(tab:s2) ). As biomass increased, we observed a decrease in snow
free date by 22 days. Stand age had a significant but positive direct relationship on snow free date
( Table \@ref(tab:s1)@ref(tab:s2) ), with snow free date increasing by 26 days. There was no
significant relationship between percent canopy closure and snow free date ( Table
\@ref(tab:s1)@ref(tab:s2) ). Topography, represented by terrain ruggedness index, had a significant
and negative direct relationship on snow free date ( Table \@ref(tab:s1)@ref(tab:s2) ), with snow
free date being observed 5 days earlier. Snow free date had a significant but negative direct
relationship on snow free duration ( Table \@ref(tab:s1)@ref(tab:s2) ), later (i.e., increase) snow
free date lead to longer snow free duration. We found significant and positive direct relationships
of both drought and stand age on snow free duration, with a stronger direct relationship between
drought and snow free duration, with snow free duration increasing by 66 days compared to 4 days (
Table \@ref(tab:s1)@ref(tab:s2) ).

Drought and snow free duration had significant direct relationships on median burn severity and burn
severity extremes ( Table \@ref(tab:s1)@ref(tab:s2) ). Drought had a negative relationship with burn
severity extremes but a positive relationship with median burn severity ( Table
\@ref(tab:s1)@ref(tab:s2) ). The relationship between snow free duration and median burn severity
was negative whereas the relationship between burn severity extremes and snow free duration was
positive ( Table \@ref(tab:s1)@ref(tab:s2) ). All forest structure metrics had no significant direct
relationship with median burn severity but all metrics had a significant direct relationship with
burn severity extremes ( Table \@ref(tab:s1)@ref(tab:s2) ). Stand age and canopy closure had a
negative relationship with burn severity extremes whereas total aboveground biomass have a positive
relationship ( Table \@ref(tab:s1)@ref(tab:s2) ).

The indirect relationship between forest structure and median burn severity - calculated by
multiplying the standardized regression coefficients of the path coefficients - showed that the
effects of stand age were stronger than canopy closure but similar to that of total aboveground
biomass, although stand age was positive whereas biomass was negative \@ref(fig:1b). In contrast,
the direct effect of canopy closure on median burn severity is stronger than both stand age and
total aboveground biomass, both of which had relatively low direct effects \@ref(fig:1b). More
importantly, the total causal effects of total aboveground biomass and canopy closure were similar
and much strong than stand age \@ref(fig:1b).

The indirect effects of forest structural diversity on burn severity extremes were strongest for
stand age compare to both total aboveground biomass and canopy closure \@ref(fig:1a). The indirect
...
total aboveground biomass were stronger compared to both stand age and canopy closure \@ref(fig:1a).
...
indirect negative relationship for median burn severity ( Figure \@ref(fig:1a) \@ref(fig:1b)). The
total cause effects of drought were positive. In contrast, topography had a positive indirect
relationship with both median burn severity and burn severity extremes, yet these relationships were
weaker than drought. However, because the indirect relationship of drought on burn median burn
severity was strongly negative and the direct relationship was strongly positive, the strength and
direction of these effects cancel each other out and the total causal effects of drought on median
burn severity to be weaker than topography (Figures \@ref(fig:1b)).
...
\@ref(tab:S3) \@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)). As biomass increased, snow free date was
28 days earlier. We found a significant positive direct relationship between stand age and snow free
date in both ecoregions (Table \@ref(tab:S3) \@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)) , with an
increase in stand age leading to earlier snow free date by 23 days in the east and 28 days in the
west. We found a significant negative direct relationship between canopy closure and snow free date
for the western boreal shield but a significant positive relationship in the eastern boreal shield
(Table \@ref(tab:S3) \@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)). Snow free date occurred 17 days
later in the east as canopy closure increased, but 27 days earlier in the west. We found a
significant negative relationship between topography and snow free date in the west, with snow free
date occuring 7 days earlier with an increase in topographic ruggedness, but not in the east (Table
\@ref(tab:S3) \@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)) . For both ecoregions we found a
significant negative relationship between snow free date and snow free duration. When snow free date
was later, snow free duration was shorter by 52 days in the east and 31 days in the west (Table
\@ref(tab:S3) \@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)). Additionally, there was a significant
positive relationship of drought and snow free duration in both ecoregions, snow free duration was
longer by 52 days in the east and 57 in the west as drought increased (Table \@ref(tab:S3)
\@ref(tab:S4) \@ref(tab:S5) \@ref(tab:S6)) .

In the eastern boreal, we found a significant negative direct relationship between snow free
duration and median burn severity but a significant positive relationship with burn severity
extremes (Table \@ref(tab:S4) \@ref(tab:S6)). We also found a significant negative direct
...
The indirect relationships of forest structure on burn severity extremes were marginal, yet similar
for all metrics in both the east and west (Table \@ref(tab:1a)). In the east, biomass had a slightly
stronger positive indirect relationship compared to the negative indirect relationships of both
canopy closure and stand age. However, canopy closure had the strongest total causal effect on burn
severity extremes. In contrast, in the west the indirect relationships of stand age were slightly
stronger than canopy closure and biomass, but the indirect relationship of stand age was negative
whereas the relationships between severity extremes, and canopy closure and biomass were both
positive. Biomass had the strongest total causal effect in the west.

Compared to burn severity extremes, the indirect relationships of forest structure on median burn
severity were marginal, and similar for all metrics in both east and west. Similar to burn severity
extremes, biomass had a slightly strong indirect relationship in the east, yet it was a negative
opposed to a positive relationship (fig:\@ref(fig2)). In the west, canopy closure and stand age had
to strongest relationship, both of which were similar although canopy closure was negative whereas
stand age was positive. In both ecoregions the total causal effects were fairly similar. Although
the total causal effects on canopy closure was stronger for extremes than median burn severity in
the east, under both measures of severity it had the strongest relationship with severity. Similarly
in the west, biomass had the strongest total causal effect, but the relationship was negative for
median burn severity whereas it was positive for burn severity extremes.
...
The indirect relationship between forest structure and heterogeneity were marginal (Figure
\@ref(fig:4), with increases in heterogeneity ranging from 1 to 3 %. Stand age had a negative
relationship, decreasing heterogeneity by 3%, whereas canopy closure and biomass were both positive,
increasing heterogeneity by 2% and 1% respectively. Unlike forest structure, the indirect
relationship between drought and burn severity heterogeneity was strong, increasing heterogeneity by
22%.

*Discussion*

Our results suggest that snow cover dynamics have a direct relationship with burn severity in
seasonally snow-covered regions that varies according to spatial scale and measure of burn severity.
At the ecozone scale snow free date decreases median burn severity but has no direct relationship
with extreme burn severity. Although fuel drying is an important driver of wildfire severity, burn
severity extremes or very high severity fires, are generally driven more by the amount of live fuels
[@parks.etal2018a]. Similarly, our results also demonstrate that fuels are more important for
high-severity fires, as the direct relationship between forest structure and burn severity is only
significant for burn severity extremes. Differences between median and extreme burn severity
suggests that wildfire severity in seasonally snow-covered regions may be driven, in part by snow
free date except in cases where fires burn at high-severity.

In the case where fires burn at high-severity, the importance of snow free date and fuel drying may
be inconsequential compared to the amount of live fuel. Because of the direct effects of forest
structure on snow accumulation and sublimation, it is possible for snow free date to be hetergeneous
across a fire perimeter[@schneiderTreeSpatialPatterns2019, @rothForestImpactsSnow2017,
@hojatimalekshah.etal2023, @balland2006, @varholaForestCanopyEffects2010]. We found that increases
in total aboveground biomass cause earlier snow free date but increases in stand age cause later
snow free date, and their combined effects may lead to greater heterogeneity in snow free date.
Therefore, it is possible that while the amount of live fuel (forest structure) may increase burn
severity extremes, it may simultaneously decrease the influence of snow free date by altering snow
accumulation and sublimation. Our study did not measure variability in snow free date across a fire
perimeter which may have an impact on patterns in both median and extreme burn severity, and may
have a stronger relationship with forest structure. Future studies on the relationship between snow
cover, forest structure and wildfire severity would benefit from such an analysis.

At the ecoregional scale, snow free date has no direct relationship with either median or extreme
burn severity. However, snow free duration has opposing effects on median and extreme burn severity
at both the ecozone and ecoregional scale, increasing extreme burn severity but decreasing median
burn severity. At the ecoregion scale,

-   more severe drought. Conditions following snow free date are important.

Indirect effects and multigroup analysis

-   reveals difference in stand composition from east to west

Explain the relationship between snow free duration and drought. As we saw in our study, snow free
duration increased as drought code increases. This is because the the later in the season fires
burn, the greater the influence of drought. Explain this relationship, in situations where snow free
duration is longer,

Why does drought code increase snow free duration.

-   provide references that support the relationship between drought influencing snow cover
...
Q2. do the strength of the direct and indirect relationships differ based on ecoregion?
...
#structural equation of multigroup
```

```{r 4, echo = FALSE, results = 'asis'}
# figure for heterogeniety indirect effects
```

# Tables

```{r s1, echo = FALSE, results = 'asis'}
source("manuscript/executable-scripts/sem-all-fires.R")
source("scripts/pub-table-function.R")
pub.table(sem_ext, response = "extreme", region = "NA")
```
...
source("manuscript/executable-scripts/sem-all-fires.R")
source("scripts/pub-table-function.R")
pub.table(sem_med, response = "median", region = "NA")
```

```{r S3, echo = FALSE, results = 'asis'}
#burn severity west/ext
source("manuscript/executable-scripts/sem-east-west.R")
source("scripts/pub-table-function.R")
pub.table(west_sem_ext, response = "extreme", region = "west")
```
...
source("manuscript/executable-scripts/sem-east-west.R")
source("scripts/pub-table-function.R")
pub.table(east_sem_ext, response = "extreme", region = "east")
```
...
source("manuscript/executable-scripts/sem-east-west.R")
source("scripts/pub-table-function.R")
pub.table(west_sem_med, response = "median", region = "west")
```
...
source("manuscript/executable-scripts/sem-east-west.R")
source("scripts/pub-table-function.R")
pub.table(east_sem_med, response = "median", region = "east")
```

```{r s7, echo = FALSE, results = 'asis'}
#heterogeneity full
source("manuscript/executable-scripts/sem-heterogeneity.R")
